<template>
  <div class="h-[calc(100vh-220px)] flex flex-col">
    <div
      class="console-detail-card rounded-xl overflow-hidden flex flex-col h-full"
    >
      <div
        class="px-4 py-3 border-b border-border-light dark:border-border-dark"
        :class="toolbarClass"
      >
      ...
      <div
        :ref="setLogsRef"
        class="flex-1 bg-zinc-900 p-4 overflow-y-auto custom-scrollbar"
      >
        <slot name="content">
          <pre
            class="text-slate-300 text-sm font-mono whitespace-pre-wrap break-all leading-6"
            >{{ logs || t('connectLogStream') }}</pre
          >
        </slot>
      </div>
    </div>
...
withDefaults(
  ...
  {
    clearButtonClass:
      'ml-auto text-slate-400 hover:text-slate-600 dark:hover:text-slate-300',
    logs: '',
    logsConnected: false,
    logsLoading: false,
