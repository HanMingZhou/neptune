...
function addRouteByChildren(
  route: RouteWithMeta | undefined,
  segments: string[] = [],
  parentName: string | null = null
): void {
  if (
    isExternalUrl(route?.path) ||
    isExternalUrl(route?.name) ||
    isExternalUrl(route?.component)
  ) {
    return
  }
  ...
  if (route?.children && route.children.length) {
    const nextSegments = isExternalUrl(route.path)
      ? segments
      : [...segments, route.path || '']
    route.children.forEach((child) =>
      addRouteByChildren(child, nextSegments, parentName)
    )
    return
  }
  ...
  if (WHITE_LIST.includes(to.name as string)) {
    if (to.name === 'Login' && token) {
      if (
        ...
        return {
          name: userStore.userInfo.authority.defaultRouter,
          replace: true
        }
      }
