...
export const isVideoExt = (url: string) => {
  const urlLower = url?.toLowerCase() || ''
  return (
    urlLower !== '' && VIDEO_EXTENSIONS.some((ext) => urlLower.endsWith(ext))
  )
}

export const isVideoMime = (type: string) => {
  const typeLower = type?.toLowerCase() || ''
  return typeLower !== '' && VIDEO_MIME_TYPES.includes(typeLower)
}

export const isImageMime = (type: string) => {
  const typeLower = type?.toLowerCase() || ''
