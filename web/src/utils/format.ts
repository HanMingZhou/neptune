...
export const formatBoolean = (bool: boolean | null) => {
    if (bool !== null) {
        return bool ? '是' : '否'
    } else {
        return ''
...
export const filterDict = (value: any, options: any[]) => {
    // 递归查找函数
    const findInOptions = (opts: any[], targetValue: any): any => {
...
export const filterDataSource = (dataSource: any[], value: any) => {
    // 递归查找函数
    const findInDataSource = (data: any[], targetValue: any): any => {
...
export const getDictFunc = async (type: string) => {
...
const colorToHex = (u: string) => {
    let e = u.replace('#', '').match(/../g)
    if (!e) return [0, 0, 0]
    const rgb: number[] = []
    for (let t = 0; t < 3; t++) rgb[t] = parseInt(e[t], 16)
    return rgb
}

const hexToColor = (u: any, e: any, t: any) => {
...
export const setBodyPrimaryColor = (primaryColor: string, darkMode: string) => {
