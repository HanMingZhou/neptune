...
function dynamicImport(
  dynamicViewsModules: Record<string, any>,
  component: string
) {
  ...
  let matchKey: string | undefined = matchKeys[0]
  if (!matchKey) {
    if (component === 'view/console/account/records.vue') {
      matchKey = keys.find((k) =>
        k.includes('view/console/account/accesslog.vue')
      )
    }
    if (
      component === 'view/console/account/settings.vue' ||
  ...
  return matchKey ? dynamicViewsModules[matchKey] : undefined
}
