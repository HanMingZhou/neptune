<template>
  <div class="space-y-6">
      ...
      >
        ...
        >
          <el-input
            ...
            class="w-full"
            @update:model-value="
              $emit('update:field', { key: 'displayName', value: $event })
            "
        ...
        >
          <el-select
            ...
            class="w-full"
            @update:model-value="
              $emit('update:field', { key: 'framework', value: $event })
            "
          >
            <el-option
            ...
            >
              <div class="flex items-center gap-3">
                <span class="material-icons text-base opacity-70">{{
                  item.icon
                }}</span>
          ...
          <div v-if="!frameworkRequired" class="text-xs text-slate-400 mt-1">
            {{ t('inference.frameworkOptionalHint') }}
          </div>
        </el-form-item>
    ...
    <div
      v-if="form.deployType === 'DISTRIBUTED'"
      class="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-xl p-6"
    >
      <h3 class="text-base font-bold mb-6 flex items-center gap-2">
        <span class="w-1 h-4 bg-primary rounded"></span>
      ...
      <div
        class="flex items-center gap-2 text-sm text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-3 py-2 rounded-lg mb-6"
      >
            ...
            <div class="create-form-stepper">
              <button
                ...
                @click="
                  $emit('update:field', {
                    key: 'workerCount',
                    value: Math.max(2, form.workerCount - 1)
                  })
                "
              >
                -
              ...
              <input
                ...
                @input="
                  $emit('update:field', {
                    key: 'workerCount',
                    value: Number($event.target.value) || 2
                  })
                "
              />
              <button
          ...
          <el-form-item :label="t('inference.autoRestart')">
            <div class="flex items-center gap-4">
              <el-switch
                ...
                @update:model-value="
                  $emit('update:field', { key: 'autoRestart', value: $event })
                "
              />
              <template v-if="form.autoRestart">
                <span class="text-xs text-slate-500">{{
                  t('inference.maxRestarts')
                }}</span>
                <div class="create-form-stepper">
                  <button
                    class="create-form-stepper__button"
                    ...
                    @click="
                      $emit('update:field', {
                        key: 'maxRestarts',
                        value: Math.max(1, form.maxRestarts - 1)
                      })
                    "
                  >
                    -
                  ...
                  <button
                    ...
                    @click="
                      $emit('update:field', {
                        key: 'maxRestarts',
                        value: Math.min(10, form.maxRestarts + 1)
                      })
                    "
                  >
                    +
      ...
      >
        <el-form-item :label="t('inference.modelVolume')" required>
          <el-select
            ...
            class="w-full"
            @update:model-value="
              $emit('update:field', { key: 'modelPvcId', value: $event })
            "
          >
            <el-option
              v-for="pvc in pvcs"
        ...
        <el-form-item :label="t('inference.modelMountPath')" required>
          <el-input
            ...
            placeholder="/model"
            @update:model-value="
              $emit('update:field', { key: 'modelMountPath', value: $event })
            "
          />
...
defineEmits<{
  'deploy-type-change': [value: InferenceDeployType]
