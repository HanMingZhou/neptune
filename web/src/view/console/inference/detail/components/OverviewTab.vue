  ...
  <div class="space-y-6">
    <div class="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-xl p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('instanceName') }}</div>
          <div class="text-sm font-medium font-mono">{{ service.instanceName }}</div>
        </div>
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('inference.framework') }}</div>
          <div class="text-sm font-medium">{{ service.framework || t('inference.customCommandMode') }}</div>
        </div>
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('deployMode') }}</div>
        ...
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('inference.authType') }}</div>
    ...
    <div class="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-xl p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        <div>
          ...
          <div class="text-2xl font-bold text-primary">{{ service.gpu || 0 }}</div>
        </div>
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('gpuModel') }}</div>
          <div class="text-sm font-medium">{{ service.gpuModel || 'CPU Only' }}</div>
        </div>
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('cpu') }} / {{ t('memory') }}</div>
    ...
    <div class="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-xl p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('inference.modelPath') }}</div>
          <div class="text-sm font-medium font-mono">{{ service.modelMountPath || '/model' }}{{ service.modelPath }}</div>
        </div>
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('inference.servicePort') }}</div>
          <div class="text-sm font-medium">{{ service.servicePort }}</div>
        </div>
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('inference.maxTokens') }}</div>
          <div class="text-sm font-medium">{{ service.maxTokens }}</div>
        </div>
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('inference.maxConcurrency') }}</div>
          <div class="text-sm font-medium">{{ service.maxConcurrency || t('inference.noLimit') }}</div>
        </div>
      </div>
      <div v-if="service.deployType === 'DISTRIBUTED'" class="mt-4 pt-4 border-t border-border-light dark:border-border-dark grid grid-cols-2 md:grid-cols-2 gap-6">
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('inference.workerCount') }}</div>
          <div class="text-sm font-medium">{{ service.workerCount }}</div>
        </div>
        <div>
          <div class="text-xs text-slate-400 mb-1">{{ t('inference.autoRestart') }}</div>
    ...
    <div v-if="service.mounts?.length" class="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-xl p-6">
      <h3 class="text-base font-bold mb-4 flex items-center gap-2">
        <span class="w-1 h-4 bg-primary rounded"></span>
      ...
      <table class="w-full text-left text-sm">
        <thead>
          <tr class="text-xs text-slate-400 font-bold border-b border-border-light dark:border-border-dark">
            <th class="px-4 py-3">PVC</th>
            <th class="px-4 py-3">{{ t('inference.mountPath') }}</th>
            <th class="px-4 py-3">{{ t('inference.subPath') }}</th>
            <th class="px-4 py-3">{{ t('readOnly') }}</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-border-light dark:divide-border-dark">
          <tr v-for="(mount, index) in service.mounts" :key="index">
            <td class="px-4 py-3 font-mono">{{ mount.pvcName }}</td>
            <td class="px-4 py-3 font-mono">{{ mount.mountPath }}</td>
            <td class="px-4 py-3 font-mono">{{ mount.subPath || '-' }}</td>
            <td class="px-4 py-3">{{ mount.readOnly ? t('yes') : t('no') }}</td>
          </tr>
        </tbody>
      </table>
    </div>

    <div v-if="service.envs?.length" class="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-xl p-6">
      <h3 class="text-base font-bold mb-4 flex items-center gap-2">
        <span class="w-1 h-4 bg-primary rounded"></span>
        {{ t('envVars') }}
      </h3>
    ...
    <div v-if="service.errorMsg" class="bg-surface-light dark:bg-surface-dark border border-red-200 dark:border-red-900 rounded-xl p-6">
      <h3 class="text-base font-bold mb-4 flex items-center gap-2 text-red-500">
        <span class="material-icons">error</span>
        {{ t('errorInfo') }}
      </h3>
      <pre class="text-sm text-red-600 dark:text-red-400 font-mono whitespace-pre-wrap break-all bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">{{ service.errorMsg }}</pre>
    </div>
  </div>
</template>
