<template>
  <div class="space-y-3 md:space-y-4">
    <div class="console-detail-card rounded-xl p-4 md:p-5 space-y-5">
      ...
      >
        ...
        <div
          ...
          class="text-sm text-amber-500 bg-amber-50 dark:bg-amber-900/20 px-3 py-2 rounded-lg flex items-center gap-2"
        >
        ...
        <template v-else>
          ...
          <div class="detail-code-surface mb-4">
            <div class="flex items-center justify-between mb-2">
              <span class="text-xs text-slate-400">{{
                t('inference.curlExample')
              }}</span>
              <button
                class="text-primary text-xs hover:underline flex items-center gap-1"
                @click="$emit('copy', curlExample)"
              >
                <span class="material-icons text-sm">content_copy</span>
                {{ t('copy') }}
              </button>
            </div>
            <code class="text-xs whitespace-pre-wrap break-all">{{
              curlExample
            }}</code>
          </div>
        ...
        <section
          ...
          :class="
            service.mounts?.length
              ? 'border-t border-border-light pt-5 dark:border-border-dark'
              : ''
          "
        >
          <h3
    ...
    >
      ...
      <pre
        class="text-sm text-red-600 dark:text-red-400 font-mono whitespace-pre-wrap break-all bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3"
        >{{ service.errorMsg }}</pre
      >
    </div>
  </div>
</template>
