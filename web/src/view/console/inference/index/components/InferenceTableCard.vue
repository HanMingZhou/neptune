    ...
    >
      ...
      >
        ...
        <tbody>
          ...
          <tr v-for="item in services" v-else :key="item.id" class="group">
            <td>
            ...
            <td>
              <span
                v-if="item.framework"
                :class="getFrameworkStyle(item.framework)"
            ...
            <td>
              <span
                :class="getDeployTypeStyle(item.deployType)"
                class="console-badge text-[11px]"
              >
                {{
                  item.deployType === 'STANDALONE'
                    ? t('standalone')
                    : t('distributed')
                }}
              </span>
...
withDefaults(
  defineProps<{
    ...
    filterStatus?: string
    getDeployTypeStyle: (deployType?: string) => string
    getFrameworkStyle: (framework?: string) => string
    ...
    pageSize?: number
    searchQuery?: string
    services?: ConsoleInferenceService[]
    total?: number
  }>(),
  {
    btnLoading: () => ({}),
...
const emit = defineEmits<{
  ...
  start: [item: ConsoleInferenceService]
  'status-change': [value?: string]
  stop: [item: ConsoleInferenceService]
}>()
