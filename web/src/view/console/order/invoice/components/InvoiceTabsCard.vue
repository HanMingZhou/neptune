        ...
        <tbody class="divide-y divide-slate-100 dark:divide-border-dark">
          ...
          >
            ...
            <td class="px-6 py-5">
              <div class="flex flex-col">
                <span
                  class="font-bold truncate max-w-[200px] text-slate-700 dark:text-slate-200"
                  >{{ invoice.title }}</span
                >
          ...
          <tr v-if="invoices.length === 0">
            <td
              colspan="5"
              class="px-8 py-12 text-center text-slate-400 text-sm"
            >
              {{ t('noData') }}
            </td>
          </tr>
...
import type { InvoiceRecord } from '@/types/order'
...
defineEmits<{
  'update:activeTab': [value: InvoiceTab]
}>()
