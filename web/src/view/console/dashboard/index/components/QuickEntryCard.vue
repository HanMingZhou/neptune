<template>
  <div class="rounded-xl border border-border-light bg-surface-light p-6 shadow-sm dark:border-border-dark dark:bg-surface-dark">
    <h3 class="mb-4 text-sm font-bold uppercase tracking-widest text-slate-400">{{ t('quickEntry') }}</h3>
    <div class="grid grid-cols-2 gap-3">
      ...
      >
        <span class="material-icons mb-2 text-primary">{{ entry.icon }}</span>
        <span class="text-[11px] font-bold">{{ t(entry.labelKey) }}</span>
      </button>
    </div>
  </div>
</template>
