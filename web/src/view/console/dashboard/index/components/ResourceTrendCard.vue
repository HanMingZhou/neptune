<template>
  <div class="rounded-xl border border-border-light bg-surface-light p-6 shadow-sm dark:border-border-dark dark:bg-surface-dark">
    <h3 class="mb-6 text-sm font-bold uppercase tracking-widest text-slate-400">{{ t('resourceTrend') }}</h3>
    <div v-if="items.length > 0" class="flex h-64 items-end justify-between px-2 pb-2">
      <div v-for="item in items" :key="item.date" class="group mx-0.5 flex flex-1 flex-col items-center gap-2">
        ...
        >
          <div class="absolute -top-7 left-1/2 -translate-x-1/2 whitespace-nowrap rounded border border-border-light bg-white px-1.5 py-0.5 text-[10px] font-bold text-primary opacity-0 shadow-sm transition-opacity group-hover:opacity-100 dark:border-border-dark dark:bg-zinc-800">
            {{ item.runningTasks }} {{ t('tasks') }}
          </div>
        </div>
        <span class="text-[10px] text-slate-500">{{ item.dateLabel }}</span>
      </div>
    </div>
