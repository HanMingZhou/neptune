...
const createDefaultForm = (): LoginFormState => ({
  username: '',
  password: '',
  email: '',
  captcha: ''
...
const createDefaultFieldErrors = (): LoginFieldErrors => ({
  username: '',
  password: '',
  email: '',
  captcha: '',
...
const getErrorMessage = (error: unknown, fallbackMessage: string): string => {
  ...
  return typedError.response?.data?.msg || typedError.message || fallbackMessage
}

const isMfaLoginResult = (value: unknown): value is LoginMfaResult => {
  return (
    typeof value === 'object' &&
    value !== null &&
    'needMfa' in value &&
...
export function useLoginPage() {
  ...
  const mfaToken = ref('')
  const mfaCode = ref('')
  const form = reactive<LoginFormState>(createDefaultForm())
  const fieldErrors = reactive<LoginFieldErrors>(createDefaultFieldErrors())
  ...
  const refreshCaptcha = async (): Promise<void> => {
    try {
      const res = (await captcha()) as ApiResponse<CaptchaData>
      if (res.code === 0) {
  ...
  const handleMfaSubmit = async (): Promise<void> => {
    const success = await userStore.MfaLoginIn(
      mfaToken.value,
      mfaCode.value.trim()
    )
    if (!success) {
  ...
  const handleRegisterSubmit = async (): Promise<void> => {
    ...
    const res = await register(payload as RegisterData)
