<template>
  <div class="w-full max-w-md">
    ...
    >
      <div
        class="relative w-12 h-12 rounded-xl overflow-hidden border border-slate-200 shadow-xl"
      >
        <NeptuneLogo class="w-full h-full scale-[1.7]" />
      </div>
      <div class="flex flex-col">
        <span class="text-2xl font-black text-slate-900 tracking-tight"
          >机器<span class="text-indigo-600">学习</span>平台</span
        >
      </div>
    </router-link>
    ...
    >
      ...
      >
        ...
        <div v-if="showMfaStep">
          <div
            class="p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-xl text-indigo-700 text-sm mb-4 flex items-center gap-2"
          >
            <svg
...
import type { LoginFieldErrors, LoginFormState } from '@/types/auth'
...
const props = withDefaults(
  defineProps<{
    ...
    errorMsg?: string
    form: LoginFormState
    fieldErrors?: Partial<LoginFieldErrors>
    isRegister?: boolean
    loading?: boolean
    mfaCode?: string
    showMfaStep?: boolean
