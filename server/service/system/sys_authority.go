...
func (authorityService *AuthorityService) CreateAuthority(auth system.SysAuthority) (authority system.SysAuthority, err error) {
	...
	if err = global.GVA_DB.Where("authority_id = ?", auth.AuthorityId).First(&system.SysAuthority{}).Error; !errors.Is(err, gorm.ErrRecordNotFound) {
		if err != nil {
			return auth, errors.Errorf("AuthorityService.CreateAuthority global.GVA_DB.First failed: %w", err)
		}
	...
	e := global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		...
		if err = tx.Create(&auth).Error; err != nil {
			return errors.Errorf("AuthorityService.CreateAuthority tx.Create failed: %w", err)
		}

		auth.SysBaseMenus = systemReq.DefaultMenu()
		if err = tx.Model(&auth).Association("SysBaseMenus").Replace(&auth.SysBaseMenus); err != nil {
			return errors.Errorf("AuthorityService.CreateAuthority tx.Association(SysBaseMenus).Replace failed: %w", err)
		}
...
func (authorityService *AuthorityService) CopyAuthority(adminAuthorityID uint, copyInfo response.SysAuthorityCopyResponse) (authority system.SysAuthority, err error) {
	...
	if err = global.GVA_DB.Where("authority_id = ?", copyInfo.Authority.AuthorityId).First(&authorityBox).Error; !errors.Is(err, gorm.ErrRecordNotFound) {
		if err != nil {
			return authority, errors.Errorf("AuthorityService.CopyAuthority global.GVA_DB.First failed: %w", err)
		}
	...
	if err != nil {
		return authority, errors.Errorf("AuthorityService.CopyAuthority MenuServiceApp.GetMenuAuthority failed: %w", err)
	}
	...
	if err != nil {
		return authority, errors.Errorf("AuthorityService.CopyAuthority global.GVA_DB.Create failed: %w", err)
	}
	...
	if err != nil {
		return authority, errors.Errorf("AuthorityService.CopyAuthority global.GVA_DB.Find failed: %w", err)
	}
	if len(btns) > 0 {
		for i := range btns {
		...
		if err != nil {
			return authority, errors.Errorf("AuthorityService.CopyAuthority global.GVA_DB.Create(btns) failed: %w", err)
		}
	}
...
func (authorityService *AuthorityService) UpdateAuthority(auth system.SysAuthority) (authority system.SysAuthority, err error) {
	...
	if err != nil {
		return system.SysAuthority{}, errors.Errorf("AuthorityService.UpdateAuthority global.GVA_DB.First failed: %w", err)
	}
	err = global.GVA_DB.Model(&oldAuthority).Updates(&auth).Error
	if err != nil {
		return auth, errors.Errorf("AuthorityService.UpdateAuthority global.GVA_DB.Updates failed: %w", err)
	}
	return auth, err
}
...
func (authorityService *AuthorityService) DeleteAuthority(auth *system.SysAuthority) error {
	err := global.GVA_DB.Debug().Preload("Users").First(&auth).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return errors.New("该角色不存在")
		}
		return errors.Errorf("AuthorityService.DeleteAuthority global.GVA_DB.First failed: %w", err)
	}
	if len(auth.Users) != 0 {
		return errors.New("此角色有用户正在使用禁止删除")
	}
	err = global.GVA_DB.Where("authority_id = ?", auth.AuthorityId).First(&system.SysUser{}).Error
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return errors.Errorf("AuthorityService.DeleteAuthority global.GVA_DB.First(SysUser) failed: %w", err)
	}
	if err == nil {
		return errors.New("此角色有用户正在使用禁止删除")
	}
	err = global.GVA_DB.Where("parent_id = ?", auth.AuthorityId).First(&system.SysAuthority{}).Error
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return errors.Errorf("AuthorityService.DeleteAuthority global.GVA_DB.First(SysAuthority) failed: %w", err)
	}
	if err == nil {
	...
	return global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		...
		if err = tx.Preload("SysBaseMenus").Preload("DataAuthorityId").Where("authority_id = ?", auth.AuthorityId).First(auth).Unscoped().Delete(auth).Error; err != nil {
			return errors.Errorf("AuthorityService.DeleteAuthority tx.Unscoped.Delete failed: %w", err)
		}

		if len(auth.SysBaseMenus) > 0 {
			if err = tx.Model(auth).Association("SysBaseMenus").Delete(auth.SysBaseMenus); err != nil {
				return errors.Errorf("AuthorityService.DeleteAuthority tx.Association(SysBaseMenus).Delete failed: %w", err)
			}
		...
		if len(auth.DataAuthorityId) > 0 {
			if err = tx.Model(auth).Association("DataAuthorityId").Delete(auth.DataAuthorityId); err != nil {
				return errors.Errorf("AuthorityService.DeleteAuthority tx.Association(DataAuthorityId).Delete failed: %w", err)
			}
		}

		if err = tx.Delete(&system.SysUserAuthority{}, "sys_authority_authority_id = ?", auth.AuthorityId).Error; err != nil {
			return errors.Errorf("AuthorityService.DeleteAuthority tx.Delete(SysUserAuthority) failed: %w", err)
		}
		if err = tx.Where("authority_id = ?", auth.AuthorityId).Delete(&[]system.SysAuthorityBtn{}).Error; err != nil {
			return errors.Errorf("AuthorityService.DeleteAuthority tx.Delete(SysAuthorityBtn) failed: %w", err)
		}
...
func (authorityService *AuthorityService) GetAuthorityInfoList(authorityID uint) (list []system.SysAuthority, err error) {
	...
	if err != nil {
		return nil, errors.Errorf("AuthorityService.GetAuthorityInfoList global.GVA_DB.First failed: %w", err)
	}
	...
	if global.GVA_CONFIG.System.UseStrictAuth {
		...
		if *authority.ParentId == 0 {
			...
			if err != nil {
				return nil, errors.Errorf("AuthorityService.GetAuthorityInfoList (strict-root) db.Find failed: %w", err)
			}
		} else {
			// 非顶级角色只能修改以下权限
			err = db.Debug().Preload("DataAuthorityId").Where("parent_id = ?", authorityID).Find(&authorities).Error
			if err != nil {
				return nil, errors.Errorf("AuthorityService.GetAuthorityInfoList (strict-non-root) db.Find failed: %w", err)
			}
		}
	} else {
		err = db.Preload("DataAuthorityId").Where("parent_id = ?", "0").Find(&authorities).Error
		if err != nil {
			return nil, errors.Errorf("AuthorityService.GetAuthorityInfoList (non-strict) db.Find failed: %w", err)
		}
	}
...
func (authorityService *AuthorityService) GetStructAuthorityList(authorityID uint) (list []uint, err error) {
	...
	if err != nil {
		return nil, errors.Errorf("AuthorityService.GetStructAuthorityList global.GVA_DB.Find failed: %w", err)
	}
...
func (authorityService *AuthorityService) CheckAuthorityIDAuth(authorityID, targetID uint) (err error) {
	...
	if err != nil {
		return errors.Errorf("AuthorityService.CheckAuthorityIDAuth GetStructAuthorityList failed: %w", err)
	}
	hasAuth := false
...
func (authorityService *AuthorityService) GetAuthorityInfo(auth system.SysAuthority) (sa system.SysAuthority, err error) {
	...
	if err != nil {
		return sa, errors.Errorf("AuthorityService.GetAuthorityInfo global.GVA_DB.First failed: %w", err)
	}
	return sa, err
}
...
func (authorityService *AuthorityService) SetDataAuthority(adminAuthorityID uint, auth system.SysAuthority) error {
	...
	if err != nil {
		return errors.Errorf("AuthorityService.SetDataAuthority global.GVA_DB.Association.Replace failed: %w", err)
	}
	return nil
}
...
func (authorityService *AuthorityService) SetMenuAuthority(auth *system.SysAuthority) error {
	...
	if err != nil {
		return errors.Errorf("AuthorityService.SetMenuAuthority global.GVA_DB.Association.Replace failed: %w", err)
	}
	return nil
}
...
func (authorityService *AuthorityService) findChildrenAuthority(authority *system.SysAuthority) (err error) {
	...
	if err != nil {
		return errors.Errorf("AuthorityService.findChildrenAuthority global.GVA_DB.Find failed: %w", err)
	}
