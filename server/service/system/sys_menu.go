...
func (menuService *MenuService) getMenuTreeMap(authorityId uint) (treeMap map[uint][]system.SysMenu, err error) {
	...
	if err != nil {
		return nil, errors.Errorf("MenuService.getMenuTreeMap global.GVA_DB.Find(SysAuthorityMenus) failed: %w", err)
	}
	...
	if err != nil {
		return nil, errors.Errorf("MenuService.getMenuTreeMap global.GVA_DB.Find(baseMenu) failed: %w", err)
	}
	...
	if err != nil {
		return nil, errors.Errorf("MenuService.getMenuTreeMap global.GVA_DB.Find(btns) failed: %w", err)
	}
...
func (menuService *MenuService) AddBaseMenu(menu system.SysBaseMenu) error {
	return global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		// 检查同一父菜单下name是否重复（不同父菜单下可以有相同的name）
		var existingMenu system.SysBaseMenu
		err := tx.Where("name = ? AND parent_id = ?", menu.Name, menu.ParentId).First(&existingMenu).Error
		if err == nil {
			return errors.New("同一父菜单下存在重复name，请修改name")
		}
		if !errors.Is(err, gorm.ErrRecordNotFound) {
			return errors.Errorf("MenuService.AddBaseMenu tx.First(existingMenu) failed: %w", err)
		}
		...
		if menu.ParentId != 0 {
			...
			if err := tx.First(&parentMenu, menu.ParentId).Error; err != nil {
				...
				return errors.Errorf("MenuService.AddBaseMenu tx.First(parentMenu) failed: %w", err)
			}
			...
			if err != nil {
				return errors.Errorf("MenuService.AddBaseMenu tx.Count(existingChildren) failed: %w", err)
			}
			...
			if existingChildrenCount == 0 {
				...
				if err != nil {
					return errors.Errorf("MenuService.AddBaseMenu tx.Count(defaultRouter) failed: %w", err)
				}
				...
				if err != nil {
					return errors.Errorf("MenuService.AddBaseMenu tx.Delete(SysAuthorityMenu) failed: %w", err)
				}
			}
		}
		...
		if err != nil {
			return errors.Errorf("MenuService.AddBaseMenu tx.Create failed: %w", err)
		}

		// 自动给超级管理员角色(888)分配新菜单，确保创建的菜单立即可见
		superAdminAuthorityID := uint(888)
		authorityMenu := system.SysAuthorityMenu{
			MenuId:      strconv.Itoa(int(menu.ID)),
			AuthorityId: strconv.Itoa(int(superAdminAuthorityID)),
		}
		if err = tx.Create(&authorityMenu).Error; err != nil {
			return errors.Errorf("MenuService.AddBaseMenu auto assign to admin failed: %w", err)
		}
...
func (menuService *MenuService) getBaseMenuTreeMap(authorityID uint) (treeMap map[uint][]system.SysBaseMenu, err error) {
	...
	if err != nil {
		return nil, errors.Errorf("MenuService.getBaseMenuTreeMap GetParentAuthorityID failed: %w", err)
	}
	...
	if global.GVA_CONFIG.System.UseStrictAuth && parentAuthorityID != 0 {
		...
		if err != nil {
			return nil, errors.Errorf("MenuService.getBaseMenuTreeMap global.GVA_DB.Find(authorityMenus) failed: %w", err)
		}
		var menuIds []string
...
func (menuService *MenuService) AddMenuAuthority(menus []system.SysBaseMenu, adminAuthorityID, authorityId uint) (err error) {
	...
	if global.GVA_CONFIG.System.UseStrictAuth && *authority.ParentId != 0 {
		...
		if err != nil {
			return errors.Errorf("MenuService.AddMenuAuthority global.GVA_DB.Find(authorityMenus) failed: %w", err)
		}
		for i := range authorityMenus {
			menuIds = append(menuIds, authorityMenus[i].MenuId)
		}
...
func (menuService *MenuService) GetMenuAuthority(info *request.GetAuthorityId) (menus []system.SysMenu, err error) {
	...
	if err != nil {
		return nil, errors.Errorf("MenuService.GetMenuAuthority global.GVA_DB.Find(SysAuthorityMenus) failed: %w", err)
	}
	...
	if err != nil {
		return nil, errors.Errorf("MenuService.GetMenuAuthority global.GVA_DB.Find(baseMenu) failed: %w", err)
	}
...
func (menuService *MenuService) UserAuthorityDefaultRouter(user *system.SysUser) {
	...
	if err != nil {
		global.GVA_LOG.Error(fmt.Sprintf("MenuService.UserAuthorityDefaultRouter Pluck failed: %v", err))
		return
	}
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			global.GVA_LOG.Error(fmt.Sprintf("MenuService.UserAuthorityDefaultRouter First failed: %v", err))
		}
	}
}
