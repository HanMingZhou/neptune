...
import (
	...
	commonResp "gin-vue-admin/model/common/response"
	"gin-vue-admin/utils/request"
	"io"
	"strings"
...
func (s *AutoCodeService) LLMAuto(ctx context.Context, llm common.JSONMap) (interface{}, error) {
	...
	if err != nil {
		return nil, errors.Errorf("大模型生成失败: %w", err)
	}
	...
	if err != nil {
		return nil, errors.Errorf("读取大模型响应失败: %w", err)
	}
	if err = json.Unmarshal(b, &resStruct); err != nil {
		return nil, errors.Errorf("解析大模型响应失败: %w", err)
	}
	if resStruct.Code == 7 { // 业务约定：7 表示模型生成失败
		return nil, errors.Errorf("大模型生成失败: %s", resStruct.Msg)
	}
