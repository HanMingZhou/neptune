...
func (dictionaryService *DictionaryService) CreateSysDictionary(sysDictionary system.SysDictionary) (err error) {
	...
	if err != nil {
		return errors.Errorf("DictionaryService.CreateSysDictionary global.GVA_DB.Create failed: %w", err)
	}
	return nil
}
...
func (dictionaryService *DictionaryService) DeleteSysDictionary(sysDictionary system.SysDictionary) (err error) {
	...
	if err != nil {
		return errors.Errorf("DictionaryService.DeleteSysDictionary global.GVA_DB.First failed: %w", err)
	}
	err = global.GVA_DB.Delete(&sysDictionary).Error
	if err != nil {
		return errors.Errorf("DictionaryService.DeleteSysDictionary global.GVA_DB.Delete failed: %w", err)
	}

	if sysDictionary.SysDictionaryDetails != nil {
		err = global.GVA_DB.Where("sys_dictionary_id=?", sysDictionary.ID).Delete(sysDictionary.SysDictionaryDetails).Error
		if err != nil {
			return errors.Errorf("DictionaryService.DeleteSysDictionary global.GVA_DB.Delete(details) failed: %w", err)
		}
		return nil
	}
	return nil
...
func (dictionaryService *DictionaryService) UpdateSysDictionary(sysDictionary *system.SysDictionary) (err error) {
	...
	if err != nil {
		return errors.Errorf("DictionaryService.UpdateSysDictionary global.GVA_DB.First failed: %w", err)
	}
	...
	if sysDictionary.ParentID != nil && *sysDictionary.ParentID != 0 {
		if err := dictionaryService.checkCircularReference(sysDictionary.ID, *sysDictionary.ParentID); err != nil {
			return errors.Errorf("DictionaryService.UpdateSysDictionary checkCircularReference failed: %w", err)
		}
	}

	err = global.GVA_DB.Model(&dict).Updates(sysDictionaryMap).Error
	if err != nil {
		return errors.Errorf("DictionaryService.UpdateSysDictionary global.GVA_DB.Updates failed: %w", err)
	}
	return nil
}
...
func (dictionaryService *DictionaryService) GetSysDictionary(Type string, Id uint, status *bool) (sysDictionary system.SysDictionary, err error) {
	...
	if err != nil {
		return sysDictionary, errors.Errorf("DictionaryService.GetSysDictionary global.GVA_DB.First failed: %w", err)
	}
	return
}
...
func (dictionaryService *DictionaryService) GetSysDictionaryInfoList(c *gin.Context, req request.SysDictionarySearch) (list interface{}, err error) {
	...
	if err != nil {
		return nil, errors.Errorf("DictionaryService.GetSysDictionaryInfoList query.Find failed: %w", err)
	}
	return sysDictionarys, nil
}
...
func (dictionaryService *DictionaryService) checkCircularReference(currentID uint, parentID uint) error {
	...
	if err != nil {
		...
		return errors.Errorf("DictionaryService.checkCircularReference global.GVA_DB.First failed: %w", err)
	}
...
func (dictionaryService *DictionaryService) ExportSysDictionary(id uint) (exportData map[string]interface{}, err error) {
	...
	if err != nil {
		return nil, errors.Errorf("DictionaryService.ExportSysDictionary global.GVA_DB.First failed: %w", err)
	}
...
func (dictionaryService *DictionaryService) ImportSysDictionary(jsonStr string) error {
	...
	return global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Create(&dictionary).Error; err != nil {
			return errors.Errorf("DictionaryService.ImportSysDictionary tx.Create failed: %w", err)
		}
		...
		if len(importData.SysDictionaryDetails) > 0 {
			...
			for _, detail := range importData.SysDictionaryDetails {
				...
				if err := tx.Create(&detailRecord).Error; err != nil {
					return errors.Errorf("DictionaryService.ImportSysDictionary tx.Create(detail) failed: %w", err)
				}
					...
					if newID, exists := idMap[detail.ID]; exists {
						if newParentID, parentExists := idMap[*detail.ParentID]; parentExists {
							if err := tx.Model(&system.SysDictionaryDetail{}).
								...
								return errors.Errorf("DictionaryService.ImportSysDictionary tx.Update(parent_id) failed: %w", err)
							}
						}
					}
				}
