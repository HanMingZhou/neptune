...
func (initDBService *InitDBService) InitDB(conf request.InitDB) (err error) {
	...
	if err != nil {
		return errors.Errorf("InitDBService.InitDB EnsureDB failed: %w", err)
	}
	...
	if err = initHandler.InitTables(ctx, initializers); err != nil {
		return errors.Errorf("InitDBService.InitDB InitTables failed: %w", err)
	}
	if err = initHandler.InitData(ctx, initializers); err != nil {
		return errors.Errorf("InitDBService.InitDB InitData failed: %w", err)
	}

	if err = initHandler.WriteConfig(ctx); err != nil {
		return errors.Errorf("InitDBService.InitDB WriteConfig failed: %w", err)
	}
	initializers = initSlice{}
	cache = map[string]*orderedInitializer{}
	return nil
...
// InitDataOnly 仅初始化数据（用于数据库已连接但数据为空的情况）
func (initDBService *InitDBService) InitDataOnly() (err error) {
	if global.GVA_DB == nil {
		return errors.New("数据库未连接")
	}

	if len(initializers) == 0 {
		return errors.New("无可用初始化过程，初始化器可能已被清空")
	}
	...
	for _, init := range initializers {
		if next, err := init.InitializeData(ctx); err != nil {
			return errors.Errorf("InitDBService.InitDataOnly InitializeData failed for %s: %w", init.InitializerName(), err)
		} else {
			ctx = next
			global.GVA_LOG.Info(fmt.Sprintf("[%s] 数据初始化成功", init.InitializerName()))
		}
	}
...
func createDatabase(dsn string, driver string, createSql string) error {
	...
	if err != nil {
		return errors.Errorf("createDatabase sql.Open failed: %w", err)
	}
	...
	if err = db.Ping(); err != nil {
		return errors.Errorf("createDatabase db.Ping failed: %w", err)
	}
	_, err = db.Exec(createSql)
	if err != nil {
		return errors.Errorf("createDatabase db.Exec failed: %w", err)
	}
	return nil
}
...
func createTables(ctx context.Context, inits initSlice) error {
	...
	for _, init := range inits {
		...
		if n, err := init.MigrateTable(next); err != nil {
			return errors.Errorf("createTables MigrateTable failed for %s: %w", init.InitializerName(), err)
		} else {
			next = n
		}
	}
