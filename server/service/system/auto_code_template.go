...
import (
	...
	utilsAst "gin-vue-admin/utils/ast"
...
func (s *autoCodeTemplate) checkPackage(Pkg string, template string) (err error) {
	...
	case "package":
		...
		if err != nil {
			return errors.Errorf("package结构异常,缺少api/v1/%s/enter.go", Pkg)
		}
		...
		if err != nil {
			return errors.Errorf("package结构异常,缺少service/%s/enter.go", Pkg)
		}
		...
		if err != nil {
			return errors.Errorf("package结构异常,缺少router/%s/enter.go", Pkg)
		}
	case "plugin":
		...
		if err != nil {
			return errors.Errorf("plugin结构异常,缺少plugin/%s/plugin.go", Pkg)
		}
	}
	return nil
}
