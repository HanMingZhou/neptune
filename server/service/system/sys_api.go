...
func (apiService *ApiService) CreateApi(api system.SysApi) (err error) {
	...
	if err != nil {
		return errors.Errorf("ApiService.CreateApi global.GVA_DB.Create failed: %w", err)
	}
	return nil
}
...
func (apiService *ApiService) GetApiGroups() (groups []string, groupApiMap map[string]string, err error) {
	...
	if err != nil {
		return nil, nil, errors.Errorf("ApiService.GetApiGroups global.GVA_DB.Find failed: %w", err)
	}
...
func (apiService *ApiService) SyncApi() (newApis, deleteApis, ignoreApis []system.SysApi, err error) {
	...
	err = global.GVA_DB.Find(&apis).Error
	if err != nil {
		return nil, nil, nil, errors.Errorf("ApiService.SyncApi global.GVA_DB.Find(apis) failed: %w", err)
	}
	var ignores []system.SysIgnoreApi
	err = global.GVA_DB.Find(&ignores).Error
	if err != nil {
		return nil, nil, nil, errors.Errorf("ApiService.SyncApi global.GVA_DB.Find(ignores) failed: %w", err)
	}
...
func (apiService *ApiService) IgnoreApi(ignoreApi system.SysIgnoreApi) (err error) {
	if ignoreApi.Flag {
		err = global.GVA_DB.Create(&ignoreApi).Error
		if err != nil {
			return errors.Errorf("ApiService.IgnoreApi global.GVA_DB.Create failed: %w", err)
		}
		return nil
	}
	err = global.GVA_DB.Unscoped().Delete(&ignoreApi, "path = ? AND method = ?", ignoreApi.Path, ignoreApi.Method).Error
	if err != nil {
		return errors.Errorf("ApiService.IgnoreApi global.GVA_DB.Delete failed: %w", err)
	}
	return nil
}
...
func (apiService *ApiService) EnterSyncApi(syncApis systemRes.SysSyncApis) (err error) {
	return global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		...
		if len(syncApis.NewApis) > 0 {
			...
			if txErr != nil {
				return errors.Errorf("ApiService.EnterSyncApi tx.Create failed: %w", txErr)
			}
		}
		for i := range syncApis.DeleteApis {
			...
			if txErr != nil {
				return errors.Errorf("ApiService.EnterSyncApi tx.Delete failed: %w", txErr)
			}
		}
		return nil
	})
...
func (apiService *ApiService) DeleteApi(api system.SysApi) (err error) {
	...
	if err != nil {
		...
		return errors.Errorf("ApiService.DeleteApi global.GVA_DB.First failed: %w", err)
	}
	err = global.GVA_DB.Delete(&entity).Error
	if err != nil {
		return errors.Errorf("ApiService.DeleteApi global.GVA_DB.Delete failed: %w", err)
	}
...
func (apiService *ApiService) GetAPIInfoList(api system.SysApi, info request.PageInfo, order string, desc bool) (list interface{}, total int64, err error) {
	...
	if err != nil {
		return apiList, total, errors.Errorf("ApiService.GetAPIInfoList db.Count failed: %w", err)
	}
	...
	if order != "" {
		...
		if !orderMap[order] {
			err = errors.Errorf("非法的排序字段: %v", order)
	...
	if err != nil {
		return apiList, total, errors.Errorf("ApiService.GetAPIInfoList db.Find failed: %w", err)
	}
	return apiList, total, err
}
...
func (apiService *ApiService) GetAllApis(authorityID uint) (apis []system.SysApi, err error) {
	...
	if err != nil {
		return nil, errors.Errorf("ApiService.GetAllApis global.GVA_DB.Find failed: %w", err)
	}

	// 路径+方法去重
	uniqueMap := make(map[string]bool)
	var deduplicated []system.SysApi
	for _, a := range apis {
		key := a.Path + ":" + a.Method
		if !uniqueMap[key] {
			deduplicated = append(deduplicated, a)
			uniqueMap[key] = true
		}
	}
	apis = deduplicated
...
func (apiService *ApiService) GetApiById(id int) (api system.SysApi, err error) {
	...
	if err != nil {
		return api, errors.Errorf("ApiService.GetApiById global.GVA_DB.First failed: %w", err)
	}
	return
}
...
func (apiService *ApiService) UpdateApi(api system.SysApi) (err error) {
	...
	if oldA.Path != api.Path || oldA.Method != api.Method {
		...
		if ferr := global.GVA_DB.First(&duplicateApi, "path = ? AND method = ?", api.Path, api.Method).Error; ferr != nil {
			if !errors.Is(ferr, gorm.ErrRecordNotFound) {
				return errors.Errorf("ApiService.UpdateApi global.GVA_DB.First(duplicateCheck) failed: %w", ferr)
			}
		} else {
	...
	if err != nil {
		return errors.Errorf("ApiService.UpdateApi global.GVA_DB.First failed: %w", err)
	}
	...
	if err != nil {
		return errors.Errorf("ApiService.UpdateApi global.GVA_DB.Save failed: %w", err)
	}
	return err
}
...
func (apiService *ApiService) DeleteApisByIds(ids request.IdsReq) (err error) {
	return global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		...
		if err != nil {
			return errors.Errorf("ApiService.DeleteApisByIds tx.Find failed: %w", err)
		}
		err = tx.Delete(&[]system.SysApi{}, "id in ?", ids.Ids).Error
		if err != nil {
			return errors.Errorf("ApiService.DeleteApisByIds tx.Delete failed: %w", err)
		}
