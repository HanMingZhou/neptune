...
func (baseMenuService *BaseMenuService) UpdateBaseMenu(menu system.SysBaseMenu) (err error) {
	...
	err = global.GVA_DB.Transaction(func(tx *gorm.DB) error {
		...
		if oldMenu.Name != menu.Name {
			// 检查同一父菜单下是否存在相同name（不同父菜单下可以有相同name）
			if !errors.Is(tx.Where("id <> ? AND name = ? AND parent_id = ?", menu.ID, menu.Name, menu.ParentId).First(&system.SysBaseMenu{}).Error, gorm.ErrRecordNotFound) {
				return errors.New("同一父菜单下存在相同name修改失败")
			}
		}
