...
func (casbinService *CasbinService) UpdateCasbin(adminAuthorityID, AuthorityID uint, casbinInfos []request.CasbinInfo) error {
	...
	if err != nil {
		return errors.Errorf("CasbinService.UpdateCasbin CheckAuthorityIDAuth failed: %w", err)
	}

	if global.GVA_CONFIG.System.UseStrictAuth {
		apis, e := ApiServiceApp.GetAllApis(adminAuthorityID)
		if e != nil {
			return errors.Errorf("CasbinService.UpdateCasbin GetAllApis failed: %w", e)
		}
		...
		for i := range casbinInfos {
			...
			if !hasApi {
				return errors.Errorf("CasbinService.UpdateCasbin: %w", errors.New("存在api不在权限列表中"))
			}
		}
	}
	...
	if !success {
		return errors.Errorf("CasbinService.UpdateCasbin: %w", errors.New("存在相同api,添加失败,请联系管理员"))
	}
	return nil
}
...
func (casbinService *CasbinService) UpdateCasbinApi(oldPath string, newPath string, oldMethod string, newMethod string) error {
	...
	if err != nil {
		return errors.Errorf("CasbinService.UpdateCasbinApi global.GVA_DB.Updates failed: %w", err)
	}

	e := utils.GetCasbin()
	err = e.LoadPolicy()
	if err != nil {
		return errors.Errorf("CasbinService.UpdateCasbinApi LoadPolicy failed: %w", err)
	}
	return nil
}
...
func (casbinService *CasbinService) RemoveFilteredPolicy(db *gorm.DB, authorityId string) error {
	err := db.Delete(&gormadapter.CasbinRule{}, "v0 = ?", authorityId).Error
	if err != nil {
		return errors.Errorf("CasbinService.RemoveFilteredPolicy db.Delete failed: %w", err)
	}
	return nil
}
...
func (casbinService *CasbinService) SyncPolicy(db *gorm.DB, authorityId string, rules [][]string) error {
	...
	if err != nil {
		return errors.Errorf("CasbinService.SyncPolicy RemoveFilteredPolicy failed: %w", err)
	}
...
func (casbinService *CasbinService) AddPolicies(db *gorm.DB, rules [][]string) error {
	...
	err := db.Create(&casbinRules).Error
	if err != nil {
		return errors.Errorf("CasbinService.AddPolicies db.Create failed: %w", err)
	}
	return nil
}
...
func (casbinService *CasbinService) FreshCasbin() (err error) {
	...
	err = e.LoadPolicy()
	if err != nil {
		return errors.Errorf("CasbinService.FreshCasbin LoadPolicy failed: %w", err)
	}
	return nil
}
