...
func (a *ApiLister) Handle(_ context.Context, _ mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		logx.Error("获取数据库API失败", logx.Field("err", err))
	...
	if err != nil {
		logx.Error("获取gin路由API失败", logx.Field("err", err))
		errorResponse := ApiListResponse{
			Success: false,
	...
	logx.Info("API列表获取成功",
		logx.Field("数据库API数量", len(databaseApis)),
		logx.Field("gin路由API数量", len(ginApis)),
		logx.Field("总数量", response.TotalCount))
