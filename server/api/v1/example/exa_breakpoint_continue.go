...
func (b *FileUploadAndDownloadApi) BreakpointContinue(c *gin.Context) {
	...
	if err != nil {
		utils.HandleError(c, err, "接收文件失败")
		return
	}
	f, err := FileHeader.Open()
	if err != nil {
		utils.HandleError(c, err, "文件读取失败")
		return
	}
...
func (b *FileUploadAndDownloadApi) BreakpointContinueFinish(c *gin.Context) {
	...
	if err != nil {
		utils.HandleError(c, err, "文件创建失败")
	} else {
...
func (b *FileUploadAndDownloadApi) RemoveChunk(c *gin.Context) {
	...
	if err = utils.RemoveChunk(file.FileMd5); err != nil {
		utils.HandleError(c, err, "缓存切片删除失败")
		return
	}
	if err = fileUploadAndDownloadService.DeleteFileChunk(file.FileMd5, file.FilePath); err != nil {
		utils.HandleError(c, err, "删除文件切片失败")
		return
	}
