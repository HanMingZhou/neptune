...
func ExtractFuncSourceByPosition(filePath string, line int) (name string, source string, startLine int, endLine int, err error) {
	...
	if readErr != nil {
		err = errors.Errorf("read file failed: %w", readErr)
		return
	}
	...
	if parseErr != nil {
		err = errors.Errorf("parse file failed: %w", parseErr)
		return
	}
	...
	if target == nil {
		err = errors.Errorf("no function encloses line %d in %s", line, filePath)
		return
	}
	...
	if start < 0 || end > len(src) || start >= end {
		err = errors.Errorf("invalid offsets for function: start=%d end=%d len=%d", start, end, len(src))
		return
	}
