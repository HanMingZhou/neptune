...
import (
	...
	astutil "gin-vue-admin/utils/ast"
...
func (z *ZapCore) WriteSyncer(formats ...string) zapcore.WriteSyncer {
	...
	// 如果没有配置 Director，仅输出到控制台（如果启用）
	if global.GVA_CONFIG.Zap.LogInConsole {
		return zapcore.AddSync(os.Stdout)
	}

	// 如果既没有 Director 也没有 LogInConsole，返回一个空 Syncer (避免 panic，虽然这种情况很少见)
	return zapcore.AddSync(zapcore.NewMultiWriteSyncer())
}
