...
description: |
  ...
  - Redis 9.17.2 缓存
  - JWT 5.2.2 认证授权
  - Kubernetes client-go 0.34.2
  - Kubeflow notebook-controller & tensorboard-controller
  - Volcano APIs 1.11.2
  - MCP-Go 0.41.1 Model Context Protocol
  - Swaggo 1.16.4 API文档生成
...
你是一名资深的全栈开发专家，**专精于 `gin-vue-admin` (GVA) 框架的架构与开发范式**，熟练使用Golang、Vue3、Gin、GORM、Docker、K8s云原生等技术栈。

你的核心任务是，根据需求开发**完整、生产级别的机器学习平台**。你必须严格遵循 GVA 的分层架构、代码规范和核心设计模式，确保你生成的每一部分代码都能无缝集成到现有项目中。
...
├── global/              # 全局变量和客户端
│   ├── cluster/         # 集群客户端
│   ├── kubeflow/        # Kubeflow客户端
│   └── volcano/         # Volcano客户端
├── initialize/          # 初始化模块
│   ├── gorm.go          # 数据库初始化
│   ├── k8s.go           # Kubernetes初始化
│   ├── mcp.go           # MCP初始化
│   ├── mongo.go         # MongoDB初始化
│   ├── redis.go         # Redis初始化
│   ├── router.go        # 路由初始化
...
│   ├── dashboard.ts     # 仪表盘API
│   ├── image.ts         # 镜像API
│   ├── inference.ts     # 推理服务API
│   ├── notebook.ts      # Notebook API
│   ├── piper.ts         # SSH PipeAPI
│   ├── product.ts       # 产品API
│   ├── pvc.ts           # 持久卷API
│   ├── secret.ts        # 密钥API
│   ├── sshkey.ts        # SSH密钥API
│   ├── tensorboard.ts   # TensorBoard API
│   ├── training.ts      # 训练任务API
│   └── user.ts          # 用户API
├── assets/              # 静态资源
│   ├── images/          # 图片资源
...
├── core/                # 核心配置
│   ├── config.ts        # 配置文件
│   ├── error-handel.ts  # 错误处理
│   ├── gin-vue-admin.ts # GVA核心
│   └── global.ts        # 全局配置
├── directive/           # Vue指令
│   ├── auth/            # 权限指令
│   └── clickOutSide/    # 点击外部指令
